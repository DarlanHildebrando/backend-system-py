...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #         serializer.save()
        #         self.stdout.write(self.style.SUCCESS(f"User created: {serializer.data["custom_user"]}"))
        #     else:
        ...
        # self.stdout.write(self.style.SUCCESS(f'Hashtags criadas: {categories_created}'))
        ...
        # self.stdout.write(self.style.SUCCESS('Criando Hashtags...'))
        # hashtags_created = Hashtag.objects.bulk_create([Hashtag(nome=h) for h in hashtags])
        # self.stdout.write(self.style.SUCCESS(f'Hashtags criadas: {hashtags_created}'))
        ...
        for enterprise in companies_data:
            ...
            if serializer.is_valid():
                ...
                self.stdout.write(self.style.SUCCESS(f'Empresa criada: {serializer.data["custom_user"]["first_name"]}'))

                self.stdout.write(self.style.WARNING(f'Criando eventos e ingressos...'))
                
                num_events = min(15, len(events_data))

                events = random.sample(events_data, num_events)
                
                for ev_to_remove in events:
                    if len(events_data) > 0:
                        print('================================')
                        events_data.remove(ev_to_remove)
                    else:
                        print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaa")    

                if count == 6 and len(companies_data) > 0:
